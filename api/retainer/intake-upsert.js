...
async function gql(q, v){
  ...
  if (!r.ok || j.errors) throw new Error(`GraphQL ${r.status}: ${j.errors ? JSON.stringify(j.errors) : await r.text()}`);
  return j.data;
}

const Q = {
  customersByEmail: `query($q:String!){ customers(first:1, query:$q){ nodes{ id email state } } }`,
  customerCreate:   `mutation($input:CustomerInput!){ customerCreate(input:$input){ customer{ id email state } userErrors{ field message } } }`,
  customerUpdate:   `mutation($input:CustomerInput!){ customerUpdate(input:$input){ customer{ id email state } userErrors{ field message } } }`,
  metafieldsSet:    `mutation($metafields:[MetafieldsSetInput!]!){ metafieldsSet(metafields:$metafields){ metafields{ namespace key type } userErrors{ field message } } }`,
  ...
  fileCreate: `mutation($files:[FileCreateInput!]!){ fileCreate(files:$files){ files{ id alt url } userErrors{ field message } } }`,
};
...
// DataURL → Shopify Files (IMAGE or FILE) → {fileId,fileUrl}
...
async function sendInviteREST(customerGid, email){
  try{
   ...
   const r=await fetch(url,{method:'POST',headers:{'X-Shopify-Access-Token':ADMIN,'Content-Type':'application/json'},body:JSON.stringify({customer_invite:{to:email||undefined}})});
...
export default async function handler(req,res){
  ...
  try{
    ...
    try{
      ...
      if (String(e).includes('ACCESS_DENIED')) return res.status(200).json({ ok:false, error:'App lacks Protected customer data. Approve the scope in your app.' });
      throw e;
    }
    ...
    const baseInput = {
      ...
      addresses: p.home_address ? [{ address1:p.home_address, firstName:p.first_name||undefined, lastName:p.last_name||undefined }] : undefined
    };
    ...
    // Build pretty lists (strings) — NOT JSON objects
